...
function showPaperDetailModal(paper) {
...
} = paper.researchTopic || (paper.coreConcepts||[]).length
    || (paper.extractionTheories||[]).length || (paper.extractionVariables||[]).length
    || (paper.relationships||[]).length || (paper.evidence||[]).length;
  ...
  function renderExtraction() {
    ...
    if ((paper.coreConcepts||[]).length) {
      html += '<div style="margin-bottom:14px"><div style="font-size:11px;font-weight:700;color:var(--accent);margin-bottom:4px">2. Core Concepts</div><div style="display:flex;gap:4px;flex-wrap:wrap">';
      paper.coreConcepts.forEach(function(c) { html += '<span style="font-size:12px;background:var(--bg);padding:2px 8px;border-radius:3px;color:var(--text-secondary)">' + escapeHtml(c) + '</span>'; });
      html += '</div></div>';
    }
    if ((paper.extractionTheories||[]).length) {
      html += '<div style="margin-bottom:14px"><div style="font-size:11px;font-weight:700;color:var(--accent);margin-bottom:4px">3. Theories</div><div style="display:flex;gap:4px;flex-wrap:wrap">';
      paper.extractionTheories.forEach(function(t) { html += '<span style="font-size:12px;background:var(--bg);padding:2px 8px;border-radius:3px;color:var(--text-secondary)">' + escapeHtml(t) + '</span>'; });
      html += '</div></div>';
    }
    if ((paper.extractionVariables||[]).length) {
    ...
    if ((paper.evidence||[]).length) {
      ...
      paper.evidence.forEach(function(e) {
  ...
  var tagsHtml = '';
  if (paper.tags && paper.tags.length) {
    tagsHtml = '<div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:8px">';
    paper.tags.forEach(function(t) { tagsHtml += '<span class="paper-tag">' + escapeHtml(t) + '</span>'; });
    tagsHtml += '</div>';
  }
  ...
  var theoryHtml = '';
  if (paper.theory) {
    theoryHtml = '<div style="font-size:12px;color:var(--text-tertiary);margin-bottom:8px">理论基础：' + escapeHtml(paper.theory) + '</div>';
  }
